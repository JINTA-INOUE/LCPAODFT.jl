...
function _Set_dDensity_Grid_woSpin!(Ngrid, Den, pccDen, Diff_Coef, dDensity_Grid)
    ...
    for xyz = 0:NN-1
        if den_min<(2*Den[1][xyz+1])

            i = div(xyz, Ngrid2*Ngrid3)
            j = div(xyz-i*Ngrid2*Ngrid3, Ngrid3)
            k = xyz - i*Ngrid2*Ngrid3 - j*Ngrid3

            p = 0
            for i1 = -1:1
                i2 = transform2primcell(Ngrid1, i+i1+1)-1
                for j1 = -1:1
                    j2 = transform2primcell(Ngrid2, j+j1+1)-1
                    for k1 = -1:1
                        k2 = transform2primcell(Ngrid3, k+k1+1)-1
            ...
            dDensity_Grid[1][1][xyz+1] = up_x
            dDensity_Grid[1][2][xyz+1] = up_y
            dDensity_Grid[1][3][xyz+1] = up_z
        else
            dDensity_Grid[1][1][xyz+1] = 0.0
            dDensity_Grid[1][2][xyz+1] = 0.0
            dDensity_Grid[1][3][xyz+1] = 0.0
        end
    end
end
...
function _Set_dDensity_Grid_wSpin!(Ngrid, Den, pccDen, Diff_Coef, dDensity_Grid)
    ...
    for xyz = 0:NN-1
        if den_min<(Den[1][xyz+1]+Den[2][xyz+1])
            ...
            dDensity_Grid[2][1][xyz+1] = dn_x
            dDensity_Grid[2][2][xyz+1] = dn_y
            dDensity_Grid[2][3][xyz+1] = dn_z
        else
            dDensity_Grid[1][1][xyz+1] = 0.0
            dDensity_Grid[1][2][xyz+1] = 0.0
            dDensity_Grid[1][3][xyz+1] = 0.0
            dDensity_Grid[2][1][xyz+1] = 0.0
            dDensity_Grid[2][2][xyz+1] = 0.0
            dDensity_Grid[2][3][xyz+1] = 0.0
        end
    end
end
...
function _Set_Vxc_GGA_PBE_wSpin!(Ngrid, Den, pccDen, dDensity_Grid, Diff_Coef, dEXC_dGD, Vxc)
    ...
    for xyz = 0:NN-1
        if (Den[1][xyz+1] + Den[2][xyz+1]) > den_min
