@timeit timer "Calc_Seebeck" function Calc_Seebeck(boltz_setup::Boltz_Setup, TDF_Energy, TDF)

    plane_type = boltz_setup.plane_type
    if plane_type
        Calc_Seebeck_2D(boltz_setup, TDF_Energy, TDF)
    else
        Calc_Seebeck_3D(boltz_setup, TDF_Energy, TDF)
    end
end
...
function Calc_Seebeck_2D(boltz_setup::Boltz_Setup, TDF_Energy, TDF)
    ...
    Nmu = TDF_EneNum
    muE = TDF_Energy  # muE is same TDF_Energy
    ...
    Seebeck = zeros(Float64, Nmu, spinsize, 4)
    ...
    for iTemp = 1:NTemp
        ...
        for spin = 1:spinsize, imu = 1:Nmu
            ...
            Seebeck[imu,spin,1] = -Seebeck22[1,1]    # xx
            Seebeck[imu,spin,2] = -Seebeck22[1,2]    # xy
            Seebeck[imu,spin,3] = -Seebeck22[2,1]    # yx
...
function Calc_Seebeck_3D(boltz_setup::Boltz_Setup, TDF_Energy, TDF)
    ...
    Seebeck = zeros(Float64, Nmu, spinsize, 9)
    ...
    for iTemp = 1:NTemp
        ...
        for spin = 1:spinsize, imu = 1:Nmu
            ...
            Seebeck[imu,spin,1] = -Seebeck33[1,1]    # xx
            Seebeck[imu,spin,2] = -Seebeck33[1,2]    # xy
            Seebeck[imu,spin,3] = -Seebeck33[1,3]    # xz
            Seebeck[imu,spin,4] = -Seebeck33[2,1]    # yx
            Seebeck[imu,spin,5] = -Seebeck33[2,2]    # yy
            Seebeck[imu,spin,6] = -Seebeck33[2,3]    # yz
            Seebeck[imu,spin,7] = -Seebeck33[3,1]    # zx
            Seebeck[imu,spin,8] = -Seebeck33[3,2]    # zy
