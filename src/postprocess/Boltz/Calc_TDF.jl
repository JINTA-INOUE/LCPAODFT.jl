@timeit timer "Calc_TDF" function Calc_TDF(
    boltz_setup::Boltz_Setup, Enk::BoltzHaloData, Vnk::BoltzHaloData,
    ...
    TDF = myrank == 0 ? similar(local_TDF) : nothing
        ...
        for cell_position = 1:halo_plan.nlocal
            @inbounds for vertex = 1:8
                ...
                for component = 1:ncomponents
                    ...
                    cell_a[vertex, component] = velocity1 * velocity2
                end
            end
            ...
            for vertices in tetra_id, component = 1:ncomponents
                ...
                first_energy = trunc(Int,
                    (tetra_e[1] - TDF_Emin) / (TDF_Emax - TDF_Emin) *
                    (TDF_EneNum - 1) - 1)
                last_energy = trunc(Int,
                    (tetra_e[4] - TDF_Emin) / (TDF_Emax - TDF_Emin) *
                    (TDF_EneNum - 1) + 1)
                   ...
                   first_energy <= last_energy
                    for ie = first_energy:last_energy
                        local_TDF[ie + 1, spin, component] +=
                            ATM_Spectrum(tetra_e, tetra_a, TDF_Energy[ie + 1])
                    end
                end
            end
        end
    ...
    if myrank == 0
        MPI.Reduce!(local_TDF, TDF, MPI.SUM, comm; root=0)
    else
    ...
    if myrank == 0
        ...
        if boltz_setup.Write_TDF
            if plane_type
                Write_TDF_3elements(boltz_setup.filename,
                    boltz_setup.mat_type, SpinPol, TDF_Energy,
                    boltz_setup.tau, TDF)
            else
                Write_TDF_6elements(boltz_setup.filename,
                    boltz_setup.mat_type, SpinPol, TDF_Energy,
                    boltz_setup.tau, TDF)
            end
        end
