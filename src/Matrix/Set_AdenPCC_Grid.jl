@timeit timer "Set_AdenPCC_Grid" function Set_AdenPCC_Grid(SpinPol::AbstractString, Init_Atoms_Nspin, Init_Atoms_Angle, pao::Vector{PAO}, pspot::Vector{Pspot}, ucell::UCell)
    ...
    for atom = 1:Natom
        ...
        for xyz = 1:GridN_Atom[atom]
            ...
            r = 0.5*log(x^2 + y^2 + z^2)


            ADen = KumoF(Spe_Num_Mesh_PAO, r, Spe_PAO_RV, Spe_Atomic_Den)
            PCCDen = KumoF(Spe_Num_Mesh_VPS, r, Spe_VPS_RV, Spe_Atomic_PCC)

            ADensity_Grid[GNc+1] += ADen
            PCCDensity_Grid[GNc+1] += PCCDen
            ...
            elseif SpinPol == "nc"
                ...
                Density_Grid[2][GNc+1] += (ocupcy_u-ocupcy_p)*ADen*sin(theta)*cos(phi)
                Density_Grid[3][GNc+1] += (ocupcy_u-ocupcy_p)*ADen*sin(theta)*sin(phi)
                Density_Grid[4][GNc+1] += (ocupcy_u-ocupcy_p)*ADen*cos(theta)
            end
        end
    end
    ...
    return ADensity_Grid/2, PCCDensity_Grid/2, Density_Grid
end
